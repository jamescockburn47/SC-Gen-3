...
def gpt_fetch_public_group_structure(
    company_number: str,
    company_name: str,
    *,
    additional_instructions: str = "",
) -> Tuple[List[Dict[str, str]], Optional[str]]:
    """Use GPT-4.1 to retrieve a list of subsidiaries from public sources.

    Returns a list of dictionaries with ``name`` and optional ``number`` keys
    along with an optional error message.
    """
    openai_client = get_openai_client()
    if not openai_client:
        logger.warning("OpenAI client unavailable; skipping GPT group lookup.")
        return [], "OpenAI client unavailable"

    system_prompt = (
        "You are a corporate research assistant. Compile a concise JSON array of "
        "known subsidiaries for the provided company from publicly available "
        "sources. Include company registration numbers if easily obtainable. "
        "Respond ONLY with a JSON array like: [{\"name\": \"ABC Ltd\", "
        "\"number\": \"12345678\"}] and no additional text."
    )
    user_content = f"Company Name: {company_name}\nCompany Number: {company_number}.\n{additional_instructions}"
    try:
        ...
        subs_data = json.loads(raw_text)
        if isinstance(subs_data, dict) and "subsidiaries" in subs_data:
            subs_data = subs_data.get("subsidiaries")
        ...
        if isinstance(subs_data, list):
            for item in subs_data:
                if isinstance(item, dict) and item.get("name"):
                    ...
                    num = item.get("number")
                    if num:
                        clean_item["number"] = str(num).strip().upper()
                    cleaned.append(clean_item)
        return cleaned, None
    except Exception as e:
        logger.error(f"{company_number}: GPT public structure lookup failed: {e}", exc_info=True)
        return [], str(e)
...
def analyze_company_group_structure(
    ...
    *,
    ...
    Set ``use_public_sources_gpt`` to fetch an initial subsidiary list from GPT-4.1.
    """
    ...
    elif use_public_sources_gpt:
        comp_name = results.get("company_profile", {}).get("company_name", company_number)
        gpt_subs, gpt_err = gpt_fetch_public_group_structure(company_number, comp_name)
        if gpt_err:
            report_messages.append(f"GPT structure lookup error: {gpt_err}")
        if gpt_subs:
            existing = {frozenset(s.items()) for s in results["subsidiary_evolution"].get(0, [])}
            for sub in gpt_subs:
                if frozenset(sub.items()) not in existing:
                    results["subsidiary_evolution"].setdefault(0, []).append(sub)
    ...
    if analysis_mode == "list_subsidiaries":
        ...
        if not st.session_state.get('gs_current_company_crn'): st.warning("Please enter a company number.") # type: ignore
            ...
            with st.spinner(f"Fetching profile for {st.session_state.get('gs_current_company_crn')}..."): # type: ignore
                ...
                results = analyze_company_group_structure(
                    company_number=st.session_state.get('gs_current_company_crn',''), api_key=api_key, base_scratch_dir=base_scratch_dir,
                ...
                if not chosen:
                    ...
                    with st.spinner("Processing selected filings..."):
                        updated_results = analyze_company_group_structure(
                            ...
                            logger,
                            ocr_handler,
                            st.session_state.get('gs_next_analysis_mode', 'list_subsidiaries'),
                            session_data=results,
                            docs_metadata_to_process=chosen,
                        )
                        next_stage = "subs_listed"
                        st.session_state.gs_analysis_results = updated_results  # type: ignore
